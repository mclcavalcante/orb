...
func TestDeleteSinkFromDataset(t *testing.T) {
	...
	for desc, tc := range cases {
		t.Run(desc, func(t *testing.T) {
			dataset, err := repo.DeleteSinkFromAllDatasets(context.Background(), tc.sinkID, tc.owner)
			assert.True(t, errors.Contains(err, tc.err), fmt.Sprintf("%s: expected '%s' got '%s'", desc, tc.err, err))
...
func TestDatasetsRetrieveByPolicyName(t *testing.T) {
	dbMiddleware := postgres.NewDatabase(db)
	repo := postgres.NewPoliciesRepository(dbMiddleware, logger)
	...
	wrongDataset.PolicyID = wrongOID.String()
	_, err = repo.SaveDataset(context.Background(), wrongDataset)
	...
	cases := map[string]struct {
		...
		"retrieve existing datasets by policy name and ownerID": {
			datasetID:  dataset.ID,
	...
	for desc, tc := range cases {
		t.Run(desc, func(t *testing.T) {
			dsList, err := repo.RetrieveDatasetsByPolicyName(context.Background(), tc.policyName, tc.ownerID)
			if err == nil {
				assert.Equal(t, tc.results, len(dsList), fmt.Sprintf("%s: expected %d got %d\n", desc, tc.results, len(dsList)))
				if tc.results > 0 {
					assert.Equal(t, dataset.ID, dsList[0].ID, fmt.Sprintf("%s: expected %s got %s\n", desc, dataset.ID, dsList[0].ID))
